...
function mapSale(s: any) {
  return {
    ...
    seller: s.operators?.name || s.users?.name || 'Sistema',
    type: 'sale',
          ...
          <section className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
            <table className="w-full text-left">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">ID</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Productos</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Op.</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Cliente</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">Vendedor</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 text-right w-[90px]">Dto.</th>
                  <th className="py-5 px-5 text-[10px] font-black uppercase tracking-[0.15em] text-slate-400 text-right w-[120px]">Total</th>
                  <th className="py-5 px-3 w-14"></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {isLoading ? (
                  <tr>
                    <td colSpan={8} className="py-20 text-center">
                      ...
                      <p className="text-sm font-black uppercase tracking-[0.3em] text-slate-300">Sincronizando...</p>
                    </td>
                  </tr>
                  ...
                  >
                      <td className="py-5 px-5">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-black text-on-surface leading-tight truncate">{sale.id}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">{formatDate(sale.created_at)}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-5 px-5 max-w-[200px]">
                        {sale.product_summary.length > 0 ? (
                          <div className="space-y-0.5">
                            {sale.product_summary.slice(0, 2).map((line: string, i: number) => (
                              <p key={i} className="text-[11px] font-semibold text-slate-500 uppercase leading-snug truncate">
                                {line}
                              </p>
                            ))}
                            {sale.product_summary.length > 2 && (
                              <p className="text-[9px] font-bold text-slate-300 uppercase">+{sale.product_summary.length - 2} más</p>
                            )}
                          </div>
                        ) : (
                      ...
                      <td className="py-5 px-5 max-w-[160px]">
                        <p className="text-xs font-bold text-on-surface uppercase leading-tight truncate">{sale.customer}</p>
                        <p className="text-[10px] font-bold text-slate-400 uppercase mt-0.5">DNI: {sale.dni}</p>
                      </td>
                      <td className="py-5 px-5 max-w-[120px]">
                        <p className="text-xs font-bold text-on-surface leading-tight truncate">{sale.seller}</p>
                      </td>
                      <td className="py-5 px-5 text-right align-middle w-[90px]">
                        {sale.discount > 0 ? (
                          <span className="inline-block px-2 py-0.5 rounded text-[10px] font-black text-error bg-error/5">
                      ...
                      <td className="py-5 px-5 text-right align-middle w-[120px]">
                        <p className="text-sm font-black font-headline italic tracking-tighter text-on-surface leading-tight">
                      ...
                      <td className="py-5 px-3 text-right align-middle w-14">
                        <div className="w-9 h-9 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-primary group-hover:text-white transition-all mx-auto">
                          <ArrowRight size={16} />
                        </div>
                      </td>
                  </tr>
                ...
                {!isLoading && filteredSales.length === 0 && (
                  <tr>
                    <td colSpan={8} className="py-40 text-center opacity-20">
