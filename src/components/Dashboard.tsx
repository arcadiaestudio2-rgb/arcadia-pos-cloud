...
export function Dashboard() {
  ...
  const [stats, setStats] = useState({ 
    ...
    paymentMix: {} as Record<string, number>,
    peakHours: [] as number[],
    avgTicket: 0,
    discountImpact: 0,
    periodTotalSales: 0
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const data = await api.fetchStats(timeFilter);
  ...
  const maxChartVal = Math.max(...stats.weeklyRevenue, ...stats.weeklyCost, 1);
  const totalMix = Object.values(stats.paymentMix).reduce((a, b) => a + b, 0) || 1;
           ...
           <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm space-y-10">
              ...
              <div className="h-80 flex items-end justify-between gap-6 px-4 relative">
                 <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-5">
                  ...
                  {(['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'].map((_, i) => {
                     const revH = (stats.weeklyRevenue[i] ?? 0) / maxChartVal * 100;
                     const costH = (stats.weeklyCost[i] ?? 0) / maxChartVal * 100;
                     return { revH, costH };
                  })).map(({ revH, costH }, i) => (
                        ...
                        <div className="w-full flex items-end gap-1.5 h-full">
                           ...
                           >
                              <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-on-surface text-white text-[10px] font-black px-3 py-1.5 rounded-xl opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100">
                                 {formatMoney(stats.weeklyRevenue[i] ?? 0)}
                              </div>
                           </motion.div>
                           <motion.div 
                              initial={{ height: 0 }}
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                     ...
                     <div className="relative w-56 h-56 rotate-[-90deg]">
                        <svg className="w-full h-full" viewBox="0 0 100 100">
                           ...
                           <motion.circle 
                             ...
                             strokeDasharray={(totalMix > 0 ? 251.2 : 251.2).toString()} 
                             strokeDashoffset={totalMix > 0 ? (251.2 * (1 - ((stats.paymentMix?.Efectivo ?? 0) + (stats.paymentMix?.CuentaCorriente ?? 0)) / totalMix)).toString() : '188.4'} 
                             strokeLinecap="round" 
                           />
                           <motion.circle 
                             cx="50" cy="50" r="40" fill="transparent" stroke="#6ffbbe" strokeWidth="15" 
                             strokeDasharray={(totalMix > 0 ? 251.2 : 251.2).toString()} 
                             strokeDashoffset={totalMix > 0 ? (251.2 * (1 - (stats.paymentMix?.Efectivo ?? 0) / totalMix)).toString() : '188.4'} 
                             strokeLinecap="round" 
                           />
                        </svg>
                        <div className="absolute inset-0 rotate-[90deg] flex flex-col items-center justify-center">
                           <span className="text-3xl font-black font-headline italic italic text-on-surface">
                              {totalMix > 0 ? Math.round(((totalMix - (stats.paymentMix?.Efectivo ?? 0)) / totalMix) * 100) : 0}%
                           </span>
                           <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em]">Digital</span>
                        </div>
                     </div>
                     <div className="w-full grid grid-cols-2 gap-4">
                        <PaymentMixItem label="Efectivo" val={totalMix > 0 ? `${Math.round(((stats.paymentMix?.Efectivo ?? 0) / totalMix) * 100)}%` : '0%'} color="bg-primary" />
                        <PaymentMixItem label="Tarjeta" val={totalMix > 0 ? `${Math.round(((stats.paymentMix?.Crédito ?? 0) / totalMix) * 100)}%` : '0%'} color="bg-tertiary" />
                        <PaymentMixItem label="Débito" val={totalMix > 0 ? `${Math.round(((stats.paymentMix?.Débito ?? 0) / totalMix) * 100)}%` : '0%'} color="bg-blue-500" />
                        <PaymentMixItem label="Transf." val={totalMix > 0 ? `${Math.round(((stats.paymentMix?.Transferencia ?? 0) / totalMix) * 100)}%` : '0%'} color="bg-sky-400" />
                        <PaymentMixItem label="Cuenta Cte" val={totalMix > 0 ? `${Math.round(((stats.paymentMix?.CuentaCorriente ?? 0) / totalMix) * 100)}%` : '0%'} color="bg-secondary" />
                     </div>
                  </div>
               </div>
               ...
               <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-sm flex flex-col">
                  ...
                  <div className="flex-1 grid grid-cols-4 grid-rows-6 gap-2">
                     {stats.peakHours.length === 24 ? Array.from({ length: 24 }).map((_, i) => {
                        const intensity = stats.peakHours[i] ?? 0;
                        const maxH = Math.max(...stats.peakHours, 1);
                        const level = intensity / maxH;
                        return (
                           <div 
                             key={i} 
                             className={`rounded-xl transition-all hover:scale-105 cursor-pointer flex items-center justify-center text-[8px] font-bold ${
                               level > 0.6 ? 'bg-primary text-white' : level > 0.2 ? 'bg-primary/30 text-primary' : 'bg-slate-50 text-slate-300'
                             }`}
                           >
                              {i}h
                           </div>
                        ...
                        <div key={i} className="rounded-xl bg-slate-50 flex items-center justify-center text-[8px] font-bold text-slate-300">{i}h</div>
                     ))}
                  </div>
                  {(() => {
                     const peakHour = stats.peakHours.length === 24 ? stats.peakHours.indexOf(Math.max(...stats.peakHours)) : -1;
                     if (peakHour >= 0) {
                        const endHour = Math.min(peakHour + 3, 23);
                        return <p className="text-[10px] font-bold text-slate-400 mt-6 uppercase text-center italic">Mejor horario para refuerzo de staff: {peakHour}:00 - {endHour}:00 hs</p>;
                     }
                     return null;
                  })()}
               </div>
           ...
           <section className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm space-y-6">
            ...
            <div className="bg-tertiary p-8 rounded-[3rem] text-white space-y-4 shadow-xl shadow-tertiary/20">
               ...
               <div className="flex justify-between items-end">
                  <div>
                     <h5 className="text-4xl font-black font-headline italic tracking-tighter">{formatMoney(stats.avgTicket)}</h5>
               ...
               <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-black text-white italic">-{stats.discountImpact}% Rev</span>
               </div>
            </div>
        </aside>
